...
const mapping = {
  ...
  changed: (node, depth) => {
    ...
    const data1 = `${stringifyKey(depth, '-', key)}${stringifyValue(value1, depth, mapping)}`;
    const data2 = `${stringifyKey(depth, '+', key)}${stringifyValue(value2, depth, mapping)}`;
